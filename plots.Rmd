...
```{r}
filename = "area_change_analysis.csv"
...
df$mean_rgb0 = (df$r0+df$g0+df$b0)/3
df$mean_rgb1 = (df$r1+df$g1+df$b1)/3
df$mean_rgb2 = (df$r2+df$g2+df$b2)/3
# plot(df$size_t0, type="l")
# points(df$size_t1, type="l", col= "red")
# points(df$size_t2, type="l", col = "blue")

# growing
sub = subset(df, size_t0<size_t1)
# bad heuristic / we actually want thin lines, not surfaces
# or do we....
# mean_size = mean(df$size_t1)
...
plot(mean_combined, type="l", ylim = c(100,150)
...
plot(mean_combined, type="l", ylim = c(80,150)
