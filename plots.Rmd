...
```{r}
filename = "fpsi_50/throat_rgb.csv"
...
for(i in c(h:0)){
  start = (i*w)+1
  y = df$lum[start:(start+w)]*f
  if(i==h){
    plot(y, type = "l", ylim = c(0,offset*h))
  } else {
    points(y+i*offset, type = "l")
  }
}

x = 59
y = 49*offset
rect(x, y, x*10, y+10, density = NULL, angle = 0, border = "red")
x = 60
y = 75*offset
rect(x, y, x+8, y+8, density = NULL, angle = 0, border = "red")
```
