...
Read interesting pixels
```{r}
dataFolder = "/Users/lana/Desktop/prgm/CSL/prednet_chainer_2/results/fpsi_50/snake_mirrored/"
```

```{r}
filename = "original/csv/00.csv"
df = read.csv(paste(dataFolder,filename,sep = ""), check.names = FALSE, stringsAsFactors=F)
colnames(df) = c("x","y","dx","dy")
original = df

filename = "mirrored/csv/00.csv"
df = read.csv(paste(dataFolder,filename,sep = ""), check.names = FALSE, stringsAsFactors=F)
colnames(df) = c("x","y","dx","dy")
mirrored = df

s = data.frame(original$dx+mirrored$dx, original$dy+mirrored$dy)
colnames(s) = c("dx", "dy")
mean(abs(df$dx))

t = 0.02
cond = original$dx > t & s$dx < t


pass = original$dx[cond]
...
m_rgb = c(mean(df$r), mean(df$g), mean(df$b))
print(m_rgb)

plot(df$r, col = "red")
points(df$g, col = "green")
points(df$b, col = "blue")
```

```{r}
filename = "previous_pixels.csv"
df = read.csv(paste(dataFolder,filename,sep = ""), check.names = FALSE, stringsAsFactors=F)
...
plot(df$g, col = "green", type = "l")
points(pix$g, type = "l")

plot(df$b, col = "blue", type = "l")
points(pix$b, type = "l")

plot(pix$r-df$r, col = "red", type = "l")
plot(pix$g-df$g, col = "green", type = "l")
plot(pix$b-df$b, col = "blue", type = "l")
```
...
Plot blur data from json file

```{r}
library("rjson")

cont <- fromJSON(file = "/Users/lana/Desktop/prgm/CSL/prednet_chainer_2/image_analysis/averages/common/fpsi_100/results_2cont.json")
proc <- fromJSON(file = "/Users/lana/Desktop/prgm/CSL/prednet_chainer_2/image_analysis/averages/common/fpsi_100/results_2proc.json")
```

```{r}
cont_data = c()
for(l in cont$results){
  cont_data = c(cont_data, l$score)
}

proc_data = c()
for(l in proc$results){
  proc_data = c(proc_data, l$score)
}

plot(proc_data, type = "l")
points(cont_data, type = "l", col = "red")
```
...
```{r}
filename = "mit_rgb_5/throat_rgb.csv"
df = read.csv(paste(dataFolder,filename,sep = ""), check.names = FALSE, stringsAsFactors=F)
```
