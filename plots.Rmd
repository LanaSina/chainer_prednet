...
displayPeaks = function(data,w,h, xr,yr,ox,oy){
...
for(x in c(1:length(xr))){
  for(i in c(yr:(yr[i]+oy[i]-1))){
    start = i*w + xr[x]
    y = data[start:(start+ox[x]-1)]*f
    points(c(xr[x]:(xr[x]+ox[x]-1)), y+i*offset, type = "l", col = "black")
  }
}
...
```{r}
filename = "mit_rgb_5/arm_luminance.csv"
...
displayPeaks(df$r, w, h, xr, yr, ox, oy)
displayPeaks(df$g, w, h, xr, yr, ox, oy)
displayPeaks(df$b, w, h, xr, yr, ox, oy)
```
